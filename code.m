...
% Define labor hours sensitivity (varying from 290 to 960 in steps of 10)
available_labor_hours = 290:10:960;

% Initialize arrays to store results
sensitivity_results = zeros(length(available_labor_hours), 1);

% Perform sensitivity analysis
for i = 1:length(available_labor_hours)
    % Update labor constraint
    prob.Constraints.c1 = w1 + w2 <= available_labor_hours(i);
    ...
    if exitflag == 1
        ...
        sensitivity_results(i) = NaN;
    end
end

% Display sensitivity analysis results
for i = 1:length(available_labor_hours)
    if ~isnan(sensitivity_results(i))
        fprintf('Total Labor Minutes Available: %d, Maximum Profit: %.2f\n', available_labor_hours(i), sensitivity_results(i));
    else
        fprintf('Total Labor Minutes  Available: %d, Solver failed to find solution.\n', available_labor_hours(i));
    end
end
